...
def post():
    ...
    start_date_bot = post_bot[1]  # дата старта бота
    is_start_date_bot = post_bot[1] == str(date.today())  # совпадает ли дата старта с сегодняшней
    is_bot_active = post_bot[2]  # активен ли бот
    post_bot_day = post_bot[3]  # день постинга у бота

    # Если бот активен и дата старта совпадает с сегодняшней
    if is_start_date_bot and is_bot_active:

        chats = DataBase.get_chats_for_post()  # Все чаты
        ...
        polls = DataBase.get_polls((False, post_bot_day))
                ...
                if post_time_init >= datetime.now().strftime(
                    ...
                    try:
                        ...
                        if 'jpg' in mediafile or 'jpeg' in mediafile:
                            bot.send_photo(chat_id=chat_num_id, photo=open(MEDIA_ROOT + mediafile, 'rb'), caption=text,
                                           parse_mode='HTML')
                            # print('Photo sent')
                        elif 'mp3' in mediafile:
                            bot.send_audio(chat_id=chat_num_id, audio=open(MEDIA_ROOT + mediafile, 'rb'), caption=text,
                                           parse_mode='HTML')
                            # print('audio sent')
                        elif 'mp4' in mediafile or 'mpeg' in mediafile or 'avi' in mediafile or 'mkv' in mediafile:
                            bot.send_video(chat_id=chat_num_id, video=open(MEDIA_ROOT + mediafile, 'rb'), caption=text,
                                           parse_mode='HTML')
                            # print('video sent')
                        else:
